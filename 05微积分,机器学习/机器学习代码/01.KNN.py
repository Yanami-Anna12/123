...
# 生成坐标点网络
x, y = np.meshgrid(
    np.linspace(axis[0], axis[1], 100), # x轴的均匀的点
    np.linspace(axis[0], axis[1], 100) # y轴的均匀的点
)
# 将 x 和 y 两个一维数组的所有坐标点组合成 N×2 的坐标点数组，每一行是一个 (x, y) 点。
axis_xy = np.c_[x.ravel(), y.ravel()]

# 4. KNN预测与绘制决策边界
y_predict = knn_clf.predict(axis_xy)
y_predict = y_predict.reshape(x.shape)
# 等高线
plt.contour(x, y, y_predict)
